...
- On `v0.1.6`, the operator observed selecting **View details** on a project appear unresponsive with four discovered projects on screen. Investigation found `selectProject` is synchronous and makes no backend call; the shared detail panel renders below the project grid and, with several projects, lands outside the current viewport with no scroll cue, so the state change (the button label switching to "Viewing details" and the card gaining a selection border) was easy to miss. `selectProject` now scrolls the detail panel into view after Vue flushes the DOM update. `docker compose run --rm verify` passed unchanged bindings (18 methods, 10 enums, 38 models) and Vue type-check/build. `scripts/build-windows.ps1 -Version 0.1.7` produced `out/penguinspace-v0.1.7.exe` (14,029,824 bytes; SHA-256 `7dbf35fcf6d2655296ae14e61293aeb589c97e4d6d5244601222254d2f59a36a`) without replacing any prior artifact.
- User-supplied Windows screenshots on `v0.1.7` against real approved roots (`AWS-First-GenAI-Journey`, then `D:\SourceCode\PROJECTS\LetterBase`) complete M4.2/M4.3 Windows acceptance for project detail, measurement, and exclusions. The scroll fix worked: selecting a project immediately brought the detail panel into view. A marker-only project with no claimed artifact (`amazon-nova-samples/multimodal-generation/image-generation/javascript`) correctly showed ecosystems, marker, Last modified, absolute path, and "No allow-listed generated directory is claimed by this project's markers, so there is nothing to measure here," with no Measure control rendered. On `LetterBase`, measuring `.next` and `node_modules` produced a project total of 1.16 GiB labelled **Full count**, per-artifact logical bytes (715 MiB / 3,297 files / 60 directories for `.next`; 475 MiB / 30,696 files / 2,444 directories for `node_modules`) with distinct per-artifact Last modified dates, Reclaimable **Unavailable** everywhere, and the modification-not-physical-reclaim boundary text displayed. Adding the exclusion `node_modules` and re-measuring correctly zeroed out that artifact's byte, file, and directory counts to **Unavailable** rather than `0`, relabelled it **Excluded scope** with the exact skip reason, relabelled the project total **Excluded scope** and shrank it to 715 MiB, listed the rule as "Applied; its bytes are not in the total," and added the warning that the excluded artifact's unknown bytes were left out of the total rather than counted as zero.
- Not yet exercised through the Windows UI: cancelling an in-flight measurement, and the background-measurement notice shown when viewing a different project while one is still being measured.
- **Deferral decision (2026-08-14):** the operator explicitly accepted cancellation and the background-measurement notice remaining fixture-only evidence rather than blocking on further Windows UI exercise. M4.4 is authorized to begin on that basis. This does not upgrade either behavior to runtime-confirmed; both stay recorded as fixture-only in [penguin-space.hallucination.md](penguin-space.hallucination.md).
