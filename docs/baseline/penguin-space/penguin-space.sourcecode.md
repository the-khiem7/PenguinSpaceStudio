...
`AppService.MeasureProjectStorage` serializes measurement under the same project mutex, requires an approved root, and applies a 120-second context. Vue adds a per-project measure control, a non-persisted exclusion textarea whose copy states that rules resolve against the approved root, per-artifact and per-project values, a count label that separates a full count from an excluded scope, an incomplete count, and a partial count, the applied-rule list, the recorded skips, and the logical-versus-physical boundary. `formatBytes` now refuses to render a value whose measurement kind is not a measured or estimated kind. No plan, confirmation, executor, prune, or deletion path exists on this surface.

## M4.3 measurement cancellation

`MeasureProject` takes an optional `*CancelSignal`, a `sync/atomic.Bool` wrapper whose zero value never cancels and whose `Cancel()` is safe to call from another goroutine. The walk checks it before every directory descent in `measureDirectory` and, inside one large listing, every 2,000 entries via `cancelCheckInterval`, so a cancel request stops within a bounded number of entries rather than only at the next directory boundary. The check runs before the context-deadline check and before the entry-budget check, so a cancel landing at the same instant as the fixed timeout or the budget is attributed to the deliberate stop rather than to an unrelated deadline or exhaustion.
