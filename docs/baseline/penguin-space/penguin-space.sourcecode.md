...
The M1 bootstrap is implemented in commit `5ba3fe8`. `main.go` hosts the Wails application, `AppService` exposes typed bindings, `internal/core` owns provider lifecycle models, and `internal/history` persists verified outcomes. Docker/WSL and project discovery remain planned. Eleven developer-tool provider slices are implemented through commit `163295b`; the minimal workspace-root contract is intentionally not a Project storage page, scanner, filter, or analytics surface. The limited elevation prototype and provider boundaries are described below.
