...
The M1 bootstrap is implemented in commit `5ba3fe8`. `main.go` hosts the Wails application, `AppService` exposes typed bindings, `internal/core` owns provider lifecycle models, and `internal/history` persists verified outcomes. Docker/WSL and project discovery remain planned. Seven developer-tool provider slices are implemented through commit `1f7098c`; Cargo, Gradle, Maven, and Playwright remain deferred until M4 provides explicit project/workspace scope. The limited elevation prototype and provider boundaries are described below.
...
The M2 implementation has the generic core `Provider` interface, an `AppService` registry, backend plan map, and seven real providers: Bun 1.x, npm 10/11, conditional pnpm 11/12, uv 0.12.x, Yarn Classic 1.x, .NET SDK 6+ NuGet HTTP cache, and Cypress 13–15 binary cache. `internal/providers/common` centralizes command execution, broad-path rejection, symlink-safe traversal, overflow checks, path equality, and Windows child-process creation with `CREATE_NO_WINDOW`. `internal/providers/managedcache` supplies the fixed lifecycle guard used by the three most recent global-cache slices: version gate, one safe validated storage root, backend-owned plan, mandatory confirmation, re-detection and same-path revalidation, tool-native command, and verified logical before/after bytes. Cargo, Gradle, Maven, and Playwright remain unimplemented: their cleanup must receive an explicit project/workspace context in M4 rather than infer or delete global/home storage.
