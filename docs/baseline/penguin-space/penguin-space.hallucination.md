...
- M3.1 and M3.3 daemon-available and daemon-unavailable Windows behavior are runtime-confirmed. M3.5 exact-network semantics are implementation- and fake-runner-verified but have not removed a live disposable network through the Windows UI. This does not authorize broader Docker cleanup or inference from labels, counts, relationships, or disk-usage summaries.
