...
M1 bootstrap is committed on `main` as `5ba3fe8` and contains `Dockerfile`, `docker-compose.yml`, Compose delegates, and a Wails/Go/Vue shell. A follow-on elevation-probe implementation is currently uncommitted after `e3f7522`. With Rancher Desktop running, Docker verification passed Bun/Vue/Wails binding generation, Go formatting/vet/tests, and a Windows cross-build; Docker build produced `out/penguinspace.exe` (13,544,960 bytes), and a hidden five-second process-start smoke test passed. The Dockerfile now sets an explicit non-login toolchain `PATH`; use `sh -c` rather than `sh -lc` in diagnostics because a login shell resets `PATH`. Interactive UI/UAC acceptance and installer/package creation remain separate work.
