...
M1 bootstrap is committed on `main` as `5ba3fe8` and contains `Dockerfile`, `docker-compose.yml`, Compose delegates, and a Wails/Go/Vue shell. The follow-on elevation-probe implementation is committed in `6ee788d`. With Rancher Desktop running, Docker verification passed Bun/Vue/Wails binding generation, Go formatting/vet/tests, and a Windows cross-build; Docker build produced `out/penguinspace.exe` (13,544,960 bytes), and a hidden five-second process-start smoke test passed. The user subsequently observed the visible no-op elevation probe reach `Succeeded`. The Dockerfile now sets an explicit non-login toolchain `PATH`; use `sh -c` rather than `sh -lc` in diagnostics because a login shell resets `PATH`. UAC refusal/cancellation/timeout, duplicate-start feedback, and installer/package creation remain separate work.
