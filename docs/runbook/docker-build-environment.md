...
PenguinSpace is a Windows-first desktop application planned for Wails 3, Go, and Vue. This runbook makes its build environment a Docker-only boundary: the Windows host is an editor and container orchestrator, not a place to install project toolchains or SDKs.

At the time this runbook was created, this repository contains the baseline documentation pack only. It has no application source, Dockerfile, or Compose manifest yet. The command names below are therefore an implementation contract, not runnable commands. Add the corresponding container files before claiming a local build has been verified.
...
- Vue frontend build tooling: Node.js package manager installs, Vite, Vue type checking, bundlers, and their package/build caches.
...
The image must pin compatible Go, Wails 3, Node.js, package-manager, and Windows cross-build dependency versions. Keep all dependency caches in Docker named volumes or container-managed paths, including at minimum:

- Go module and build caches;
- Node/package-manager download cache and `node_modules` when practical;
...
```powershell
# Names are a required interface to implement, not commands available yet.
npm run verify
...
Before considering the bootstrap complete, verify that a clean Windows host without Go, Wails, Node build tooling, or Windows build SDKs can run the Docker verification command. Record the actual commands and results in the baseline documentation pack once implemented.
...
- [ ] Dockerfile and Compose manifest are committed.
- [ ] Go, Wails 3, Vue/Node, and Windows packaging dependencies execute only in containers.
- [ ] Go, package-manager, and build caches stay out of the host profile and source checkout.
- [ ] `verify`, `build:windows`, and `shell` delegate only to Docker.
- [ ] A clean Windows host completes the Docker verification path.
