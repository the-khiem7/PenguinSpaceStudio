...
M1 bootstrap is committed on `main` as `5ba3fe8` and contains `Dockerfile`, `docker-compose.yml`, Compose delegates, and a Wails/Go/Vue shell. The elevation probe is implemented across `6ee788d`, `8fc559a`, and the post-consent execution timing fix `d0bc468`. The current uncommitted M2 working tree adds Bun 1.x, npm 10/11, a pnpm 11/12 provider limited to an explicit `storeDir`, and uv 0.12.x. With Rancher Desktop running, final Docker verification passed Bun/Vue/Wails binding generation, Go formatting/vet/tests for shared/Bun/npm/pnpm/uv provider packages, and a Windows cross-build. Docker build produced `out/penguinspace.exe` (13,655,040 bytes; SHA-256 `ea6416d9a68f63d429c150d3903e556738ff945d36e1c85dcf2381bcfbf46e6b`). The rebuilt M1 executable had already passed visible Windows auto-consent, cancellation, timeout, and 3.00 MiB fixture-lifecycle smoke tests. M2 Windows acceptance detected Bun 1.3.13, npm 11.8.0, pnpm 11.3.0, and uv 0.12.1. Bun, npm, and uv opened explicit cleanup confirmation and were cancelled without deleting user caches; pnpm exposed no action for its implicit per-disk stores. Isolated fixtures verified Bun and npm 4 KiB scope, npm `_logs` exclusion, pnpm prune from 21,903 to 12,288 logical bytes, and uv removal of 4 KiB inside its cache while preserving a 2 KiB sentinel outside it. The real uv inspection measured 3.45 GiB; its initial recursive measurement timed out and was replaced with official `uv cache size`. The uv run also exposed a transient Windows Terminal window, so the shared runner now uses `CREATE_NO_WINDOW`. Tests, Windows compile-only checks, final Docker verification, and build pass after that patch; the final interactive no-console recheck was interrupted before launch and is the exact resume point. The host's UAC level is Never notify, so `runas` auto-approved and refusal could not be exercised; that runtime case requires a session that presents a prompt. This host has unrelated Go, Bun, Node/npm, pnpm, uv, and .NET installations, so it cannot supply clean-host evidence even though every PenguinSpace build command used Docker. The Dockerfile sets an explicit non-login toolchain `PATH`; use `sh -c` rather than `sh -lc` in diagnostics because a login shell resets `PATH`. Installer/package creation remains separate work.
...
M1 is complete with the clean-host Docker verification explicitly deferred to M6. Before packaging or release, a clean Windows host without Go, Wails, Bun build tooling, or Windows build SDKs must run the documented Docker verification command. M1 recorded container verification plus visible Windows success, cancellation, timeout, and fixture-lifecycle acceptance; interactive UAC refusal is separately deferred to M3 before real privileged execution.
...
- [ ] M6 release gate: a clean Windows host completes the Docker verification path. M1 deferral approved 2026-08-12; container verification passed, but clean-host evidence does not yet exist.
